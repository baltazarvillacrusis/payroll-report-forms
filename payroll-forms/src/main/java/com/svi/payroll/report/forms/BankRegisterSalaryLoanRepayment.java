...
public class BankRegisterSalaryLoanRepayment {
	...
	private final int LOAN_NAME_COLINDEX = 5;
	private final int AMOUNT_COLINDEX = 6;
	...
	public BankRegisterSalaryLoanRepayment() {
	}

	public BankRegisterSalaryLoanRepayment(EmployeeBean employeeBean) {
		this.employeeBean = employeeBean;

	}

	public BankRegisterSalaryLoanRepayment(EmployeeBean employeeBean, String period, String companyName) {
		this.employeeBean = employeeBean;
		this.period = period;
	...
	public void createExcelStream(FileOutputStream outputStream) {
		...
		try {
			...
			for (Employee employee : employeeBean) {
				if (employee.getSalaryLoans() != null) {
					for (Map.Entry<String, BigDecimal> entry : employee.getSalaryLoans().entrySet()) {
						// writing employee ID
						...
						// writing bank name
						writeData(rowNumber, LOAN_NAME_COLINDEX, sheet, entry.getKey(), cellStyle1);
						// writing bank name
						writeData(rowNumber, AMOUNT_COLINDEX, sheet, entry.getValue().doubleValue(), cellStyle3);
						totalAmount = totalAmount + entry.getValue().doubleValue();
						rowNumber++;
					}
				}
			}
